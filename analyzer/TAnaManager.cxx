...
// #endif
  ...
  fV1290Histogram = 0;
#ifdef USE_V1290
  fV1290Histogram = new TV1290Histograms();
  fV1290Histogram->DisableAutoUpdate();  // disable auto-update.  Update histo in AnaManager.
#endif
...
#ifdef USE_FREQ
  ffreqmon = new freqmon();
  ffreqmon->DisableAutoUpdate();
#endif
...
//   if(fAgilentHistogram)  fAgilentHistogram->UpdateHistograms(dataContainer);
  ...
  if (ffreqmon)  ffreqmon->UpdateHistograms(dataContainer);
...
bool TAnaManager::HaveV1290Histograms() {
  if (fV1290Histogram) return true;
  return false;
};
...
bool TAnaManager::Havefreqmon() {
  if (ffreqmon) return true;
  return false;
};
...
freqmon* TAnaManager::Getfreqmon() {return ffreqmon;}
